...
from unreal_psk import UnrealPsk
...
def find_chunk(psk, name):
	for chunk in psk.chunks:
		if chunk.name == name:
			return chunk
	return None

# load psk
psk = UnrealPsk.from_file(sys.argv[1])
is_animation = True if psk.chunks[0].name == "ANIMHEAD" else False

# write skeleton
skl = None
if is_animation:
	skl = find_chunk(psk, "BONENAMES")
else:
	skl = find_chunk(psk, "REFSKELT")

# create bone tree
bones = {}
for i,bone in enumerate(skl.entries):
	bones[bone.name] = Bone(bone.name, i, resolve_bone_name(skl.entries, bone.parent_index - 1), bone.transform.position, bone.transform.orientation, bone.transform.scale)
...
def remap_bone_index(index):
	return bones[resolve_bone_name(skl.entries, index)]
...
for bone in bonelist:
	...
	print(f"writing bone \"{bone.name}\"")
	outfile.write(f"joint \"{bone.name}\" {parent_index}\n")
	if is_animation:
		continue
	outfile.write(f"\tpq {bone.translate.x} {bone.translate.y} {bone.translate.z}")
	outfile.write(f" {bone.rotate.x} {bone.rotate.y} {bone.rotate.z} {bone.rotate.w}")
	outfile.write(f" {bone.scale.x} {bone.scale.y} {bone.scale.z}\n")

if is_animation:
	# write animations
	animinfo = find_chunk(psk, "ANIMINFO")
	animkeys = find_chunk(psk, "ANIMKEYS")
	for animation in animinfo.entries:
		print(f"writing {animation.name}")
else:
	pnts0000 = find_chunk(psk, "PNTS0000")
	vtxw0000 = find_chunk(psk, "VTXW0000")
	face0000 = find_chunk(psk, "FACE0000")
	matt0000 = find_chunk(psk, "MATT0000")
	rawweights = find_chunk(psk, "RAWWEIGHTS")

	# write vertices
	for vertex in vtxw0000.entries:
		outfile.write(f"vp {pnts0000.entries[vertex.point].x} {pnts0000.entries[vertex.point].y} {pnts0000.entries[vertex.point].z}\n")
		outfile.write(f"\tvt {vertex.u} {vertex.v}\n")
